...
// GCC triggers both. To workaround this clang bug, we have to manage const correctness ourselves
...
// pointer to const member function - reference to object
// can be invoked on any cv-qualified object
template <
    typename Base,
    typename T,
    typename Derived,
    ...
    typename std::enable_if<
        is_function<T>::value && is_const_member_function<T>::value &&
...
// pointer to non-const member function - reference to object
// can only be invoked on non-const objects
template <
    typename Base,
    typename T,
    typename Derived,
    ...
    typename std::enable_if<
        is_function<T>::value && !is_const_member_function<T>::value &&
            !std::is_const<
                typename std::remove_reference<Unwrapped>::type>::value &&
            std::is_base_of<Base, typename std::decay<Unwrapped>::type>::value,
        int>::type = 0>
inline auto invoke_impl(T Base::*pmf, Derived&& ref, Args&&... args)
    FPLUS_INVOKE_RETURN((std::forward<Unwrapped>(ref).*
...
// pointer to const member function - pointer to object
// can be invoked on any cv-qualified object
template <
    typename Base,
    typename T,
...
// pointer to non-const member function - pointer to object
// can only be invoked on non-const objects
template <
    typename Base,
    typename T,
    ...
    typename std::enable_if<
        is_function<T>::value && !is_const_member_function<T>::value &&
            !std::is_const<typename std::remove_reference<
                decltype(*std::declval<Pointer>())>::type>::value &&
            !std::is_base_of<Base, typename std::decay<Pointer>::type>::value,
        int>::type = 0>
inline auto invoke_impl(T Base::*pmf, Pointer&& ptr, Args&&... args)
    FPLUS_INVOKE_RETURN(((*std::forward<Pointer>(ptr)).*
